 ...
 */
function dggb_render_block_core_latest_posts($attributes)
{
    /**
     * Global post object.
    ...
    if ($grid_query->have_posts()) {
        ...
        while ($grid_query->have_posts()) {
            ...
            $post_classes = 'dggb-post-grid-item';
            ...
            if (isset($attributes['displayPostImage']) && $attributes['displayPostImage'] && $post_thumb_id) {
                ...
                $post_grid_markup .= sprintf(
                    '<div class="dggb-block-post-grid-image"><a href="%1$s" rel="bookmark" aria-hidden="true" tabindex="-1">%2$s</a></div>',
            ...
            $post_grid_markup .= sprintf(
                '<div class="dggb-block-post-grid-text">'
            );

            $post_grid_markup .= sprintf(
                '<header class="dggb-block-post-grid-header">'
            );
            ...
            if (isset($attributes['displayPostTitle']) && $attributes['displayPostTitle']) {
                ...
                $post_grid_markup .= sprintf(
                    '<%3$s class="dggb-block-post-grid-title"><a href="%1$s" rel="bookmark">%2$s</a></%3$s>',
                    esc_url(get_permalink($post_id)),
                    esc_html($title),
            ...
            if (isset($attributes['postType']) && $attributes['postType'] === 'post') {
                ...
                $post_grid_markup .= sprintf(
                    '<div class="dggb-block-post-grid-byline">'
                );

                /* Get the post author */
                if (isset($attributes['displayPostAuthor']) && $attributes['displayPostAuthor']) {
                    $post_grid_markup .= sprintf(
                        '<div class="dggb-block-post-grid-author" itemprop="author" itemtype="https://schema.org/Person"><a class="dggb-text-link" href="%2$s" itemprop="url" rel="author"><span itemprop="name">%1$s</span></a></div>',
                ...
                if (isset($attributes['displayPostDate']) && $attributes['displayPostDate']) {
                    $post_grid_markup .= sprintf(
                        '<time datetime="%1$s" class="dggb-block-post-grid-date" itemprop="datePublished">%2$s</time>',
            ...
            $post_grid_markup .= sprintf(
                '<div class="dggb-block-post-grid-excerpt">'
            );
            ...
            if (isset($attributes['displayPostLink']) && $attributes['displayPostLink']) {
                $post_grid_markup .= sprintf(
                    '<p><a class="dggb-block-post-grid-more-link dggb-text-link" href="%1$s" rel="bookmark">%2$s <span class="screen-reader-text">%3$s</span></a></p>',
                    esc_url(get_permalink($post_id)),
        ...
        $class = "dggb-block-post-grid featured{$attributes['postType']} align{$attributes['align']}";
        ...
        $grid_class = 'dggb-post-grid-items';
        ...
        if (isset($attributes['displaySectionTitle']) && $attributes['displaySectionTitle'] && !empty($attributes['sectionTitle'])) {
            ...
            $section_title = '<' . esc_attr($section_title_tag) . ' class="dggb-post-grid-section-title">' . esc_html($attributes['sectionTitle']) . '</' . esc_attr($section_title_tag) . '>';
        } else {
            $section_title = null;
        }
 ...
 */
function dggb_register_block_core_latest_posts()
{
    ...
    register_block_type(
        ...
        array(
            ...
            'render_callback' => 'dggb_render_block_core_latest_posts',
        )
    );
}
add_action('init', 'dggb_register_block_core_latest_posts');

/**
 * Create API fields for additional info
 */
function dggb_register_rest_fields()
{
    ...
    register_rest_field(
        ...
        'featured_image_src',
        array(
            'get_callback' => 'dggb_get_image_src_landscape',
    ...
    register_rest_field(
        ...
        array(
            'get_callback' => 'dggb_get_image_src_square',
            'update_callback' => null,
            'schema' => null,
        )
    );
    ...
    register_rest_field(
        ...
        'author_info',
        array(
            'get_callback' => 'dggb_get_author_info',
            'update_callback' => null,
            'schema' => null,
        )
    );
}
add_action('rest_api_init', 'dggb_register_rest_fields');
 ...
 */
function dggb_get_image_src_landscape($object, $field_name, $request)
{
    $feat_img_array = wp_get_attachment_image_src(
        $object['featured_media'],
        'dggb-block-post-grid-landscape',
        false
    );
 ...
 * @param String $request  The current request object.
 */
function dggb_get_image_src_square($object, $field_name, $request)
{
    $feat_img_array = wp_get_attachment_image_src(
        $object['featured_media'],
        'dggb-block-post-grid-square',
        false
    );
    if (is_array($feat_img_array)) {
        return $feat_img_array[0];
 ...
 * @param String $request  The current request object.
 */
function dggb_get_author_info($object, $field_name, $request)
{
    /* Get the author name */
